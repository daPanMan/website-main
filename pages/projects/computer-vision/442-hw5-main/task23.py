...
def find_triangulation(K1, K2, F, pts1, pts2):
    ...
    Returns:
    - pcd: Numpy array of shape (4,N) giving the homogeneous 3D point cloud
      data
    """
    pcd = None
    ###########################################################################
    ...
    # M1 = np.dot(K1, np.hstack((np.eye(3), np.zeros((3, 1)))))
    # M2 = np.dot(K2, np.hstack((R, t)))
    ...
    # Ensure the scene is not mirrored
    ...
    pcd[:3, :] = np.dot(rotation_matrix, pcd[:3, :])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return pcd
