...
def image_patches(image, patch_size=(16, 16)):
    ...
    output = []
    for j in range(0, image.shape[0], patch_size[0]):
        for i in range(0, image.shape[1], patch_size[1]):
            incJ = patch_size[0]
            if j+patch_size[0] >= image.shape[0]:
                continue
            incI = patch_size[1]
            if i+patch_size[1] >= image.shape[1]:
                continue
            patch = image[j:j+incJ, i:i+incI]
...
def convolve(image, kernel):
    ...
    for j in range(0, image.shape[0]):
        for i in range(0, image.shape[1]):
            ...
            #         print(b, a, j, i)
            #         sum+=kernel[b][a]*padded[j: j+kernel.shape[0], i: i+kernel.shape[1]][b][a]
            if(kernel.shape == padded[j: j+kernel.shape[0], i: i+kernel.shape[1]].shape):
                output[j, i] = (kernel*padded[j: j+kernel.shape[0], i: i+kernel.shape[1]]).sum()
...
def edge_detection(image):
    ...
    # grad_magnitude = np.empty(Ix.shape)
    # for j in range(Ix.shape[0]):
    #     for i in range(Ix.shape[1]):
    #         print(grad_magnitude[j, i])
    #         print(Ix[j, i])
    #         print(Iy[j, i])
    #         grad_magnitude[j, i] = np.sqrt(Ix[j, i]^2 + Iy[j, i]^2)
...
def main():
    ...
    #     save_img(chosen_patches[i], f"./image_patches/q{i}_patch.png")
    ...
    #print(np.mean(edge_detect == edge_with_gaussian))
