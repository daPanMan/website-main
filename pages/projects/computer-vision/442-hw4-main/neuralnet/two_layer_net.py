...
class TwoLayerNet(Classifier):
    ...
    def forward(self, X):
        ...
        scores, cache2 = fc_forward(out1_relu, self.params['W2'], self.params['b2'])
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
        return scores, (cache1, cache1_relu, cache2)
    ...
    def backward(self, grad_scores, cache):
        ...
        #######################################################################
        cache1, cache1_relu, cache2 = cache
        grad_out1_relu, grad_W2, grad_b2 = fc_backward(grad_scores, cache2)
        grad_out1 = relu_backward(grad_out1_relu, cache1_relu)
        _, grad_W1, grad_b1 = fc_backward(grad_out1, cache1)
